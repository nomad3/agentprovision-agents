...
def test_paid_cli_fast_pin_set_includes_oss_lane_b_clis():
    """Wave 1c review I1 + Wave 2d B3: the explicit-platform fast-pin
    set must include every paid / OAuth-gated CLI surface — original
    four (claude_code, codex, gemini_cli, copilot_cli) plus the OSS
    Lane B coding CLIs (qwen_code, kimi_k2) and Wave 2d's BYOK
    multi-provider CLI (goose). Missing entries cause a paid tenant
    to silently downgrade to local Gemma 4 on a transient resolver
    hiccup — the regression PR #245 review C5 closed.
