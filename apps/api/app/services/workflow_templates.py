...
NATIVE_TEMPLATES = [
    ...
    },
    {
        ...
        "description": "Brett cardiology MVP: read a Drive packet folder, extract referral PDFs, draft a DACVIM-style review packet, save it to Drive, and stop at veterinarian approval.",
        "tier": "native",
        "public": True,
        "tags": ["healthpets", "vet", "cardiac", "echocardiogram", "google-drive", "report", "approval"],
        "trigger_config": {"type": "manual"},
        "definition": {
            "steps": [
                {
                    "id": "list_drive_packet",
                    "type": "mcp_tool",
                    "tool": "search_drive_files",
                    "params": {
                        "query": "'{{input.drive_folder_id}}' in parents and trashed=false",
                        "max_results": 20,
                ...
                {
                    "id": "read_referral_summary",
                    "type": "mcp_tool",
                    "tool": "read_drive_file",
                    "params": {
                        "file_id": "{{input.referral_summary_file_id}}",
                        "account_email": "{{input.account_email}}",
                    },
                    "output": "referral_summary",
                },
                {
                    "id": "read_echo_measurements",
                    "type": "mcp_tool",
                    "tool": "read_drive_file",
                    "params": {
                        "file_id": "{{input.echo_pdf_file_id}}",
                        "account_email": "{{input.account_email}}",
                    },
                    "output": "echo_measurements",
                },
                {
                    "id": "read_report_template",
                    "type": "mcp_tool",
                    "tool": "read_drive_file",
                    "params": {
                        "file_id": "{{input.report_template_file_id}}",
                        "account_email": "{{input.account_email}}",
                    },
                    "output": "report_template",
                },
                {
                    "id": "read_invoice_context",
                    "type": "mcp_tool",
                    "tool": "read_drive_file",
                    "params": {
                        "file_id": "{{input.invoice_file_id}}",
                        "account_email": "{{input.account_email}}",
                    },
                    "output": "invoice_context",
                },
                {
                    "id": "generate_cardiac_report",
                    "type": "agent",
                    "agent": "Cardiac Diagnostics Agent",
                    "prompt": "Create a veterinarian-review draft for the Brett cardiology referral workflow.\n\nPatient: {{input.patient_name}}\nVisit date: {{input.visit_date}}\nDrive packet files: {{packet_files.files}}\n\nReferral summary:\n{{referral_summary.content}}\n\nEcho measurements/source PDF:\n{{echo_measurements.content}}\n\nReport template:\n{{report_template.content}}\n\nBilling/context document:\n{{invoice_context.content}}\n\nReturn markdown only. Required sections: Packet inventory, source fields extracted, echo measurement table, draft cardiac report, missing/low-confidence fields, and approval checklist. Do not invent missing measurements. Do not state that the report was sent. Mark the document as DRAFT - veterinarian review required.",
                    "output": "report",
                },
                {
                    "id": "save_review_draft",
                    "type": "mcp_tool",
                    "tool": "create_drive_file",
                    "params": {
                        "name": "TEST DRAFT - Cardiology Packet - {{input.patient_name}} - {{input.visit_date}}.md",
                ...
                },
                {
                    "id": "brett_approval",
                    "type": "human_approval",
                    "name": "Dr. Brett approval required before send-back",
                    "timeout_hours": 72,
                    "on_timeout": "reject",
                    "output": "approval",
                },
            ...
            "input_schema": {
                "patient_name": {"type": "string", "description": "Patient name (e.g. Winnie Nieto)"},
                "visit_date": {"type": "string", "description": "Visit date (e.g. 2026-02-14)"},
                "account_email": {"type": "string", "description": "Connected Google account email"},
                "drive_folder_id": {"type": "string", "description": "Google Drive folder ID containing the referral packet and output draft"},
                "referral_summary_file_id": {"type": "string", "description": "Drive file ID for the referral summary PDF"},
                "echo_pdf_file_id": {"type": "string", "description": "Drive file ID for the echo/source measurements PDF"},
                "report_template_file_id": {"type": "string", "description": "Drive file ID for the report template PDF"},
                "invoice_file_id": {"type": "string", "description": "Drive file ID for the invoice/context PDF"},
            },
        },
    },
    {
