...
If nothing is high or medium priority, respond with: []"""
    ...
    try:
        from app.services.local_inference import triage_inbox_items as _gemma_triage
        gemma_result = await _gemma_triage(items_text)
        if gemma_result is not None:
            logger.info("triage_items: used local Gemma 4 model (saved Anthropic tokens)")
            return gemma_result
        logger.debug("Gemma 4 triage returned None — falling back to Anthropic")
    except Exception as e:
        logger.debug("Gemma 4 triage failed (%s) — falling back to Anthropic", e)
