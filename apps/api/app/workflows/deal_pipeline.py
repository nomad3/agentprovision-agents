...
class DealPipelineWorkflow:
    ...
    async def run(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        skip_discovery = params.get("skip_discovery", False)
        provided_ids = params.get("prospect_ids", [])
        ...
        if skip_discovery and provided_ids:
            # Webhook-triggered: we already have prospect IDs, skip discover+score
            prospect_ids = provided_ids
            high_scorer_ids = provided_ids
            workflow.logger.info(f"Skipping discovery, using {len(provided_ids)} provided prospect IDs")
        else:
            # Step 1: Discover
