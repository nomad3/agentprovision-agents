...
def _screen_persona_prompt(persona_prompt: Optional[str]) -> None:
    ...
    if verdict.decision == "block":
        ...
        # message they'd see if a USER hit it.
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=verdict.to_refusal_message() + (
                " (persona_prompt write-time screen — see "
                "docs/superpowers/specs/2026-05-22-subproject-a-"
                "infra-secret-hardening-design.md F11)"
            ),
        )
