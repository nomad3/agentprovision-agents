"""Ad platform tools for campaign management across Meta, Google, and TikTok.

Uses stored credentials from the credential vault to call ad platform APIs
on behalf of the authenticated tenant. Also provides public ad library/transparency
search functions that do not require authentication.
"""
import logging
...
# ---------------------------------------------------------------------------
# Shared HTTP client (for internal API calls to credential vault)
# ---------------------------------------------------------------------------
...
async def _get_ads_credentials(tenant_id: str, integration_name: str) -> Optional[dict]:
    ...
    Args:
        ...
        integration_name: Integration key (e.g., "meta_ads", "google_ads", "tiktok_ads").

    Returns:
        Dict of credential fields or None if unavailable.
    """
...
async def list_meta_campaigns(
    ...
    try:
        ...
        if e.response.status_code == 403:
            return {"error": "Insufficient permissions on this Meta ad account. Check app permissions."}
        if e.response.status_code == 429:
            return {"error": "Meta API rate limit reached. Please try again in a few minutes."}
        error_body = e.response.text[:300]
        return {"error": f"Meta API error ({e.response.status_code}): {error_body}"}
...
async def get_meta_campaign_insights(
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            row = insights[0]
            # Extract conversion actions if present
            conversions = {}
            for action in row.get("actions", []):
                action_type = action.get("action_type", "")
                conversions[action_type] = action.get("value", "0")
...
async def search_meta_ad_library(
    ...
    """Search the Meta Ad Library for active ads. This is a PUBLIC API — no authentication required.
    ...
    # Meta Ad Library API requires an access token even for public queries,
    # so we provide the direct URL as a reliable fallback.
    fallback_url = (
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code != 200:
                # API likely requires token — return fallback URL
                return {
                    "status": "fallback",
                    "message": (
        ...
        return {
            "status": "fallback",
            "message": (
                "Meta Ad Library API is unavailable. "
                "Use the web URL below, or ask web_researcher to scrape it."
            ),
            "url": fallback_url,
            "query": query,
            "country": country,
...
async def _refresh_google_access_token(refresh_token: str) -> Optional[str]:
    ...
    if not client_id or not client_secret:
        logger.warning("google_client_id/google_client_secret not configured in ADK settings")
        return None
...
async def list_google_campaigns(
    ...
    """List Google Ads campaigns using the Google Ads API searchStream.
    ...
    if not access_token:
        return {"error": "Failed to refresh Google access token. Check google_client_id/secret in server config and refresh_token in credentials."}
    ...
    try:
        ...
        if e.response.status_code == 403:
            return {"error": "Insufficient permissions for this Google Ads account. Check developer token and account access."}
        if e.response.status_code == 429:
...
async def get_google_campaign_metrics(
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            spend = total_cost_micros / 1_000_000  # micros to currency
...
async def search_google_ads_transparency(
    ...
    """Search the Google Ads Transparency Center. This is PUBLIC — no authentication required.

    Note: The Google Ads Transparency Center does not have a public API.
    This function returns the search URL for a web_researcher agent to scrape.
...
async def list_tiktok_campaigns(
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if data.get("code") != 0:
                error_msg = data.get("message", "Unknown TikTok API error")
                return {"error": f"TikTok API error: {error_msg}"}
...
async def search_tiktok_creative_center(
    ...
    """Search the TikTok Creative Center for ad inspiration. This is PUBLIC — no authentication required.

    Note: The TikTok Creative Center does not provide a public API.
    This function returns the search URL for a web_researcher agent to scrape.
