...
async def generate_cardiac_report(
    patient_data: str,
    exam_data: str,
    echo_data: str,
    measurements: str = "{}",
    ...
    """Generate a complete cardiac evaluation report in DACVIM format.

    Produces a professional cardiology report matching the format used by
    board-certified veterinary cardiologists (DACVIM).

    Args:
        patient_data: JSON with name, species, breed, age, sex, weight, medications, medical_notes
        exam_data: JSON with heart_rate, resp_rate, murmur_grade, murmur_type, murmur_location, lungs, bcs, presenting_complaint, referral diagnostics
        echo_data: JSON with echo_summary text and any AI-generated findings
        measurements: JSON with echo_measurements (2d, mmode, doppler sub-objects)
        staging: JSON with acvim_stage, hcm_stage, assessment, plan
        tenant_id: Tenant context

    Returns:
        Dict with report_markdown (complete report in DACVIM format) and status.
    """
    tenant_id = _resolve_tenant_id(tenant_id)

    patient = _parse_json(patient_data, {})
    exam = _parse_json(exam_data, {})
    echo = _parse_json(echo_data, {})
    meas = _parse_json(measurements, {})
    stage = _parse_json(staging, {})

    prompt = f"""You are a board-certified veterinary cardiologist (DACVIM-Cardiology) generating a professional cardiac evaluation report.

Generate a complete cardiac evaluation report in markdown using the data below. Follow the exact structure shown in the example.
...
{json.dumps(echo, indent=2)}

Echo Measurements:
{json.dumps(meas, indent=2)}

Staging & Assessment:
{json.dumps(stage, indent=2)}

Required report structure (follow this format exactly):
...
## Exam
### Physical Exam
[Narrative describing auscultation findings, murmur grade/type/location, lung sounds, BCS]

### Echocardiogram
[Echo narrative summary followed by ACVIM or HCM stage]

### Echo Measurements
| Parameter | Value |
|-----------|-------|
[Table with all available measurements organized by 2D, M-Mode, Doppler sections]

## Assessment
### Problem List
[Numbered list of cardiac diagnoses and findings]

### Concurrent Conditions
[Any non-cardiac conditions noted]

## Plan
### Medications
[Current and recommended medications with dosages]

### Anesthesia/Fluid/Steroid Risk
[Risk assessment for anesthesia, fluid therapy, steroids]

### Diagnostics
[Recommended follow-up diagnostics and timeline]

---
*Report generated by AI-assisted cardiac analysis*

Rules:
- Use today's date in the header
- Only include sections where data is available
- Be precise with measurements — include units
- Use professional veterinary cardiology terminology
- Write the physical exam and echo sections as clinical narratives, not bullet points
- The measurements table should have clear section headers (2D, M-Mode, Doppler)
- Include ACVIM stage for dogs or HCM stage for cats in the echo section
...
Return the complete report in markdown format."""
    ...
    try:
        ...
        logger.exception("Cardiac report generation failed: %s", e)
        return {"status": "error", "error": str(e)}
