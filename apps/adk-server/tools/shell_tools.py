"""Shell execution tools for self-modifying dev team agents.

Provides controlled shell command execution and git-based deploy workflows.
Used by dev team agents to run build commands, tests, linting, and push
...
async def execute_shell(
    ...
    Args:
        command: The shell command to execute (e.g. "python -m pytest tests/").
        working_dir: Working directory for the command. Defaults to "/app".
        timeout: Maximum seconds before the command is killed. Defaults to 60,
            maximum 300.
...
async def deploy_changes(
    ...
    try:
        ...
        if files:
            for f in files:
                stage_result = await execute_shell(f"git add {f}", working_dir=working_dir, timeout=30)
            ...
            stage_result = await execute_shell("git add -A", working_dir=working_dir, timeout=30)
            if stage_result["return_code"] != 0:
                return {
                    "status": "error",
                    "step": "stage",
        ...
        diff_result = await execute_shell(
            "git diff --cached --name-only", working_dir=working_dir, timeout=30
        )
        files_changed = [
        ...
        commit_result = await execute_shell(
            f"git commit -m '{safe_message}'", working_dir=working_dir, timeout=30
        )
        ...
        # Extract commit SHA
        sha_result = await execute_shell(
            "git rev-parse --short HEAD", working_dir=working_dir, timeout=10
        )
        commit_sha = sha_result["stdout"].strip()

        # Push to main
        push_result = await execute_shell(
            "git push origin main", working_dir=working_dir, timeout=120
        )
