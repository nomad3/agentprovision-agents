...
// Default engine is `markdown` since the python/shell paths are unused in
// production (0 executions in 7 days as of 2026-04-25). Skills are now
// authored as prompt content that the CLI agent reads, not callable scripts.
const EMPTY_SKILL = { name: '', description: '', engine: 'markdown', category: 'general', auto_trigger: '', tags: '', chain_to: [], script: ENGINE_DEFAULTS.markdown, inputs: [] };
...
const SkillsPage = () => {
  ...
  const filtered = skills
    // Two scopes only — bundled (read-only, ships with platform) and the
    // tenant's own. The legacy `community` pool is archived and not surfaced.
    .filter(s => activeTab === 'native' ? s.tier === 'native' : s.tier === 'custom')
