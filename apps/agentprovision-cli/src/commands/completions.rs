//! `ap completions <shell>` — emit shell completion script to stdout.
//!
...
pub async fn run(args: CompletionsArgs, ctx: Context) -> anyhow::Result<()> {
    ...
    let hint = match args.shell {
        Shell::Bash => "ap completions bash > /etc/bash_completion.d/ap   # or ~/.bash_completion",
        Shell::Zsh => {
            "ap completions zsh > \"${fpath[1]}/_ap\"   # then `autoload -U compinit; compinit`"
        }
        Shell::Fish => "ap completions fish > ~/.config/fish/completions/ap.fish",
        Shell::PowerShell => "ap completions powershell | Out-String | Invoke-Expression",
        Shell::Elvish => "ap completions elvish > ~/.config/elvish/lib/ap.elv",
