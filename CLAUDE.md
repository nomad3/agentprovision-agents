...
- **Governance** (NOT a single table — `agent_policies` was removed in P0b 2026-05-23 as dead infrastructure, zero rows and zero enforcement). Distributed across: `agent.tool_groups` → JWT scope → MCP scope check in `apps/mcp-server/src/tool_audit.py` (tool permissions); `platform_safety_io` (content gating, regex/classifier safety floor); `core.rate_limit.limiter` (per-endpoint rate limits); AgentValueSet (per-(tenant,agent) declared values, `docs/plans/2026-05-21-luna-value-layer-design.md`); Value Arbitration (plural-source reconciliation, `docs/plans/2026-05-23-value-arbitration-design.md`). Migration 097 (create) + 147 (drop) both in history.
...
- ~~`agent_policy.py`~~ — REMOVED in P0b 2026-05-23 (migrations 097 create + 147 drop, both in history). The model was dead infrastructure with zero enforcement call sites and zero rows in production. Governance concerns now distributed: tool permissions via `agent.tool_groups` + MCP scope check; content gating via `platform_safety_io`; rate limits via `core.rate_limit.limiter`; declared values via AgentValueSet; cross-source arbitration via Value Arbitration. See `docs/plans/2026-05-23-p0b-agent-policy-decision.md` for the deletion rationale.
