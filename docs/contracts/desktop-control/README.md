...
| `grant_request.pending.json` | P5.4b/P5.5 pending desktop approval request (`alpha desktop grant request`) | reduced/display-safe: ids/status/action/capability/reduced target only; observe requests carry no target bundle, pointer/keyboard requests carry a bundle; mirrors `DesktopGrantRequestOut` (API) / `DesktopGrantRequest` (core); records intent, never actuates |
