...
| **Phase 1.5 — user_signal upstream wire-in (PENDING)** | — | — | not started | Per Luna 2026-05-22 audit P0 #1: PR 5 ships the `appraise_and_record_user_signal` contract but the caller (`cli_session_manager` chat-write hot path) is dormant. Until this lands the `pursue`-match PAD scaling is unobservable in production. Tracked as task #336. |
