...
**Status:** plan — pending council review
**Builds on:** `docs/plans/2026-05-20-whatsapp-waha-migration-design.md` (the `AbstractWhatsAppBackend` interface, `channel_accounts.backend` column, and webhook-receiver shape are reused, not redone)
...
Split the single neonize socket into **two HTTP halves** behind the `AbstractWhatsAppBackend` seam. **The fire-and-forget chat pipeline in the middle is reused verbatim** — the backend only changes *where a message arrives* and *how the reply leaves*.
...
- `POST` validates `X-Hub-Signature-256` = HMAC-SHA256 over the **raw body** keyed by the App Secret (⚠️ re-serializing the JSON breaks the HMAC), returns **200 immediately**, **dedups by `wamid`** via a Redis SET (24h TTL — Meta retries up to 7 days on non-200), then normalizes `entry[].changes[].value.messages[]` into the same dict shape `MessageEv` produces today.
- **Codebase seam:** split `whatsapp_service.py:_handle_inbound` (~line 1190) into `_normalize_inbound_event(source) -> InboundMessage` + a neonize-free `_handle_inbound(dict)`. Media is already downloaded synchronously (~line 1228) — **keep that** (Cloud API media URLs expire ~15 min; download via `GET /<MEDIA_ID>` → short-lived URL → fetch with bearer).
...
**Per-tenant `ChannelAccount` repurposed (not replaced):** add `backend VARCHAR(20) DEFAULT 'neonize'` (the WAHA design already reserves it; Cloud API = a third value `'cloud-api'`), plus `waba_id`, `phone_number_id`, `access_token` (Fernet-encrypted), `token_expires_at`. **Keep `session_blob` during cutover for rollback.**
...
## 6. Migration phases (build on the WAHA design)

**P0 — interface + single pilot number, direct Cloud API, behind a flag (~1 sprint).** Reuse/confirm `whatsapp_backends/base.py` (`AbstractWhatsAppBackend`) + `NeonizeBackend` wrapping current code (zero behavior change); migration adds `channel_accounts.backend` (default `'neonize'`) + `waba_id`/`phone_number_id`/`access_token`/`token_expires_at`. Build `CloudAPIBackend` (httpx sender + cloud-webhook receiver) for **one pilot number we control** (own Meta App + System User token, **manual provisioning — no Embedded Signup yet**). `select_backend` returns `'cloud-api'` only for the pilot, gated by a `tenant_features` flag. Split `_handle_inbound`. Pre-register 2-3 templates. Validate full send/receive + template path.
...
- **Product:** the 24h/template gate may **silently break existing proactive Luna behaviors** — inventory every current proactive send path before cutover, or users perceive Luna as "gone quiet." Template approval latency/rejection (wrong category auto-reject; utility→marketing auto-reclass) can block launch — author conservatively, pre-submit early. Quality downgrade + preemptive enforcement can restrict the number if cadence is too high — throttle/opt-in before multi-tenant send.
