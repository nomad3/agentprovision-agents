...
set -euo pipefail
read -r STDIN
TOOL=$(jq -r '.tool_input.tool_name // ""' <<<"$STDIN")
curl -fsS -m 2 -X POST \
...
- Both wrappers are deterministic templates; tests in `apps/code-worker/tests/test_hook_template_generation.py` assert the rendered output matches a golden snapshot.
...
| `agent dispatch <id> --goal "…"` | `POST` | `/api/v1/tasks` (`task_type="delegate"`, `target_agent_id=<id>`) | ♻ reuse | `agent_tasks.py:22` |
...
| **code** |  |  |  |  |
| `code task "<description>"` | `POST` | `/api/v1/tasks` (`task_type="code"`, `task_description="…"`) | ♻ reuse | `agent_tasks.py:22` *(dispatches `CodeTaskWorkflow` server-side)* |
| `code task-show <task-id>` | `GET` | `/api/v1/tasks/{task_id}` | ♻ reuse | `agent_tasks.py:120` |
| `code task-trace <task-id>` | `GET` | `/api/v1/tasks/{task_id}/trace` | ♻ reuse | `agent_tasks.py:147` |
| **memory** |  |  |  |  |
| `memory recall "<query>"` | `POST` | `/api/v1/mcp` *(JSON-RPC `tools/call` → `recall_memory`)* | ♻ reuse | `mcp_bridge.py:52` — no REST `/memory/*` endpoint; recall lives in `memory_continuity` MCP module and the CLI calls it through the JSON-RPC bridge |
| `memory record-observation` | `POST` | `/api/v1/mcp` *(JSON-RPC `tools/call` → `record_observation`)* | ♻ reuse | same |
...
| **tool / mcp** |  |  |  |  |
| `tool list` | `POST` | `/api/v1/mcp` *(JSON-RPC `tools/list`)* | ♻ reuse | `mcp_bridge.py:52` *(single JSON-RPC entrypoint, not REST per-tool)* |
| `tool call <name>` | `POST` | `/api/v1/mcp` *(JSON-RPC `tools/call`)* | ♻ reuse | same |
| **internal — Phase 4 only** |  |  |  |  |
| (leaf hook) PostToolUse heartbeat | `POST` | `/api/v1/agents/internal/heartbeat` | 🆕 new | resolves review I9 — accepts agent-token JWT only; payload `{agent_id, task_id, parent_workflow_id, tool_name, ts}`; updates in-flight task last-seen-at; emits `event` trigger when missed |

### §10.1 Summary

- **✅ shipped**: 31 rows — every CLI subcommand against an endpoint that already exists with the correct path and verb
- **♻ reuse**: 6 rows — endpoint exists but the prior draft proposed a new shape; reusing instead. Two of these (memory recall, record-observation) route through the existing MCP JSON-RPC bridge at `/api/v1/mcp` rather than a REST surface that doesn't exist.
- **🆕 new**: 1 row — `/api/v1/agents/internal/heartbeat` (Phase 4)

### §10.2 Auth model per row

All `✅`/`♻` rows accept tenant JWT (the existing path used by the web SPA). The single 🆕 endpoint accepts **agent-token JWT only** (kind=agent_token), per §8 tenancy precedence. Service-to-service paths (`/internal/*`) are out of scope for the CLI.
