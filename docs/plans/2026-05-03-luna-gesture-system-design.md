...
- **Cursor task batching** — recognizer spawns one Tokio task per debounced `point` frame (~12.5 Hz given the 80 ms debounce). Phase 4 switches to a single watcher task fed by `tokio::sync::watch` for backpressure.
- **Two-step destructive confirm** — `WakeMachine::set_confirm_pending` is wired but unused; `recognizer` and the JS dispatcher don't trigger it. Destructive action kinds (`memory_clear`, `workflow_dismiss`) currently no-op in the JS switch. Phase 4 wires the confirm flow + adds those handlers, OR removes destructive kinds from the recorder dropdown.
- **Engine starts pre-login** — the engine boots in `lib.rs::run()::setup` regardless of auth state. Camera + Apple Vision cycles burn during the login screen. Phase 4 gates engine start on a successful login or explicit user toggle.
- **Delete confirmation in bindings list** — `GestureBindingRow.jsx` deletes immediately on click. Phase 4 adds a confirm step.
